...
func (handler *DiscordHandlerImpl) RunBot6B(username string, password string) error {
	...
	driver, err := selenium.NewRemote(caps, fmt.Sprintf("http://%s:%s/wd/hub", handler.SeleniumConfig.ChromeDriverIP, handler.SeleniumConfig.SeleniumPort))
	if err != nil {
	...
	if len(presentButton) != 0 {
		if err := handler.RunBot6B(username, password); err != nil {
